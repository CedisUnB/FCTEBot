# rag_service.py (Versão Corrigida e Verdadeiramente Paralela)
import os
import logging
import uvicorn
import asyncio
...
# --- MUDANÇA: Funções síncronas que serão executadas em threads ---

def _get_relevant_chunks_sync(query: str) -> str:
    """
    Função SÍNCRONA para buscar no Pinecone.
    É a mesma lógica de antes, mas em uma função síncrona dedicada.
    """
...
#     before_sleep=lambda s: logger.warning(f"Rate limit atingido ou erro na API. Tentando novamente... (Tentativa {s.attempt_number})")
# )
...
# --- API FastAPI (com as correções) ---

app = FastAPI(title="RAG Service for FCTEBOT")
...
async def responder_endpoint(request: QueryRequest):
    """
    Recebe uma pergunta, busca contexto e gera uma resposta de forma.
    """
    query = request.query
    ...
    try:
        contexto = await asyncio.to_thread(_get_relevant_chunks_sync, query)
        
        prompt_final = make_prompt(query, contexto)
        
        resposta = await asyncio.to_thread(_generate_content_sync, prompt_final)
        
        return QueryResponse(answer=resposta)
    except Exception as e:
        logger.error(f"Um erro inesperado ocorreu ao processar a query '{query}': {e}", exc_info=True)
